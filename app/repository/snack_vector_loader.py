...
async def load_db_data_to_vectordb(
    ...
    while snack_count < limit // 2:
        ...
        logger.info(
            f"DB에서 과자 데이터 배치 {offset}-{offset+len(snack_data_batch)} 로드 완료"
        )
        ...
        for snack in snack_data_batch:
            ...
            allergy_list = (
                json.loads(snack.get("allergy_list", "[]"))
                if isinstance(snack.get("allergy_list"), str)
                else snack.get("allergy_list", [])
            )
            safe_food_mark_list = (
                json.loads(snack.get("safe_food_mark_list", "[]"))
                if isinstance(snack.get("safe_food_mark_list"), str)
                else snack.get("safe_food_mark_list", [])
            )
            ...
            if isinstance(additives, str):
                try:
                    additives = json.loads(
                        additives.replace("{{", "[").replace("}}", "]")
                    )
                except:
                    additives = []
            ...
            if isinstance(additive_descriptions, str):
                try:
                    additive_descriptions = json.loads(
                        additive_descriptions.replace("{{", "[").replace("}}", "]")
                    )
                except:
            ...
            if isinstance(additive_grades, str):
                try:
                    additive_grades = json.loads(
                        additive_grades.replace("{{", "[").replace("}}", "]")
                    )
                except:
                    additive_grades = []
            ...
            if isinstance(additive_main_uses, str):
                try:
                    additive_main_uses = json.loads(
                        additive_main_uses.replace("{{", "[").replace("}}", "]")
                    )
                except:
                    additive_main_uses = []
            ...
            if isinstance(same_company_products, str):
                try:
                    same_company_products = json.loads(
                        same_company_products.replace("{{", "[").replace("}}", "]")
                    )
                except:
            ...
            if isinstance(same_type_products, str):
                try:
                    same_type_products = json.loads(
                        same_type_products.replace("{{", "[").replace("}}", "]")
                    )
                except:
                    same_type_products = []
            ...
            use_summary = (
                ", ".join([f"{use}({count})" for use, count in top_uses])
                if top_uses
                else "첨가물 용도 정보 없음"
            )
            ...
            for i, additive in enumerate(additives):
                if i < len(additive_descriptions):
                    ...
                    grade = (
                        additive_grades[i] if i < len(additive_grades) else "정보 없음"
                    )
...
포함된 첨가물:{additives_info if additives_info else ' 첨가물 정보 없음'}
                ...
                metadata={
                    ...
                    "top_additive_uses": (
                        [use for use, _ in top_uses] if top_uses else []
                    ),
                    "similar_company_products": (
                        same_company_products[:5] if same_company_products else []
                    ),
                    "similar_type_products": (
                        same_type_products[:5] if same_type_products else []
                    ),
                    "prefix": "snack",
    ...
    while additive_count < limit // 2:
        ...
        logger.info(
            f"DB에서 첨가물 데이터 배치 {offset}-{offset+len(additive_data_batch)} 로드 완료"
        )
        ...
        for additive in additive_data_batch:
            # 주요 용도 리스트 파싱
            main_use_list = (
                json.loads(additive.get("main_use_list", "[]"))
                if isinstance(additive.get("main_use_list"), str)
                else additive.get("main_use_list", [])
            )
...
안전성 메시지: {additive.get('stability_message', '정보 없음')}
    ...
    logger.info(
        f"DB에서 과자 및 첨가물 데이터 로드 완료 (과자: {snack_count}, 첨가물: {additive_count})"
    )
...
async def save_documents_batch(session, documents, batch_size):
    ...
    for i in range(0, len(documents), batch_size):
        ...
        logger.info(
            f"문서 배치 {i}-{i+len(batch)}/{len(documents)} 벡터 DB에 저장 완료"
        )


async def insert_documents_to_vectordb(session: SessionDep, documents: List[Document]):
