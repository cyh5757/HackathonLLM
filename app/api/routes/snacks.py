...
async def get_snack_test(
    *,
    ...
    result: list[SnackDetailDto] = await snack_repository.get_snacks_without_allergy_keyword(
        session,
        snack_types.AllergyKeyword.대두
    )

    await session.commit()

    return SimpleResponseMessage(
        message=f"HI {str(result)}"
    )
...
async def ask_query(
    *,
    ...
    async def stream_llm_response(prompt: str):
        async for chunk in agent_tools.llm.astream(prompt):  # 비동기 제너레이터라고 가정
    ...
    *,
    ...
    return StreamingResponse(
        sse_stream_generator(
            get_rag_response_with_context(session, request.query)
        ),
...
async def test_context_only_query(
    *,
    ...
    """
    query: str = request.query
    result = await rag_service_test.get_context_only(
        session=session,
        query=query
    )
